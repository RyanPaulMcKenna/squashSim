...
class RobotGUI(Sofa.Core.Controller):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.artic = kwargs["articulations_mo"]
    ...
    def onAnimateEndEvent(self, event):
        # show actual solved angles in the sliders to avoid tug-of-war feeling
        q = [float(x) for x in self.artic.position.value]  # force plain floats

        self.app.root.after(0, lambda:
        ( self.app.angle1.set(q[0]),
          self.app.angle2.set(q[1]),
          self.app.angle3.set(q[2]),
          self.app.angle4.set(q[3]),
          self.app.angle5.set(q[4]),
          self.app.angle6.set(q[5]) ))
